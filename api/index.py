...
def check_and_reassign_dead_leads():
    try:
        # Throttle: Only run if it hasn't run in the last 1 hour
        routing_ref = db.collection("settings").document("lead_routing")
        routing_doc = routing_ref.get()
        now = datetime.now(timezone.utc)
        if routing_doc.exists:
            last_check_str = routing_doc.to_dict().get("last_dead_leads_check")
            if last_check_str:
                try:
                    last_check = datetime.fromisoformat(last_check_str.replace("Z", "+00:00"))
                    if (now - last_check).total_seconds() < 3600: # 1 hour
                        return
                except Exception:
                    pass
        
        # Save check timestamp immediately to prevent concurrent executions
        routing_ref.set({"last_dead_leads_check": now_iso()}, merge=True)
        
        # 1. Fetch all leads with status == "dead"
        dead_leads = db.collection("leads").where("status", "==", "dead").stream()
        
        # Get all active salespersons
        sales_docs = db.collection("users").where("role", "==", "sales").get()
        sales_users = [d.to_dict() for d in sales_docs if d.to_dict().get("is_active") is not False]
        if not sales_users:
            return
            
        for doc in dead_leads:
            lead = doc.to_dict()
            if lead.get("permanently_dead") is True:
                continue
                
            dead_at_str = lead.get("dead_at")
            if not dead_at_str:
                continue
                
            try:
                dead_at = datetime.fromisoformat(dead_at_str.replace("Z", "+00:00"))
                # Has it been 7 days (1 week)?
                if (now - dead_at).total_seconds() >= 7 * 24 * 3600:
                    dead_by_uids = lead.get("dead_by_uids", [])
                    
                    # Available salespersons who haven't marked this lead dead yet
                    eligible_sales = [u for u in sales_users if u["id"] not in dead_by_uids]
                    
                    if eligible_sales:
                        # Sort by ID to ensure consistent round-robin order
                        eligible_sales.sort(key=lambda u: u["id"])
                        
                        # Round-robin selection based on last_assigned_uid
                        next_idx = 0
                        last_uid = None
                        if routing_doc.exists:
                            last_uid = routing_doc.to_dict().get("last_assigned_uid")
                            
                        if last_uid:
                            found = False
                            for i, u in enumerate(eligible_sales):
                                if u["id"] == last_uid:
                                    next_idx = (i + 1) % len(eligible_sales)
                                    found = True
                                    break
                            if not found:
                                for i, u in enumerate(eligible_sales):
                        ...
                        next_sales = eligible_sales[next_idx]
                        
                        # Reassign lead
                        doc.reference.update({
                            "assigned_to": next_sales["id"],
                            "assigned_to_name": next_sales["name"],
                            "status": "new",
                            "dead_at": None,
                            "updated_at": now_iso(),
                            "notes": firestore.ArrayUnion([{
                                "text": f"Lead automatically reassigned to {next_sales['name']} after 1 week in dead status.",
                                "author": "System",
                                "timestamp": now_iso()
                            }])
                        })
                        ...
                        # Marked dead by all active sales employees! Mark permanently dead.
                        doc.reference.update({
                            "permanently_dead": True,
                            "updated_at": now_iso(),
                            "notes": firestore.ArrayUnion([{
                                "text": "Lead marked dead by all sales employees. Marked permanently dead.",
                                "author": "System",
                                "timestamp": now_iso()
                            }])
                        })
            except Exception as ex:
                logger.error(f"Error checking/reassigning dead lead {lead.get('id')}: {ex}")
    except Exception as ex:
        logger.error(f"Global check_and_reassign_dead_leads error: {ex}")
...
def put_object(path: str, data: bytes, content_type: str) -> dict:
    if IS_VERCEL:
        ...
        return {"path": path, "size": len(data), "firebase": True}
    else:
...
def create_access_token(uid: str, email: str, role: str) -> str:
    payload = {"sub": uid, "email": email, "role": role, "exp": datetime.now(timezone.utc) + timedelta(days=7)}
...
def send_email(to_email: str, subject: str, body: str):
    if not SMTP_USER or not SMTP_PASS:
        logger.warning(f"⚠️ SMTP NOT CONFIGURED. OTP for {to_email}: {body}")
        return
    try:
        logger.info(f"📧 Attempting to send email to {to_email} via {SMTP_HOST}...")
        msg = MIMEMultipart()
        msg['From'] = EMAIL_FROM
        msg['To'] = to_email
        msg['Subject'] = subject
        ...
        with smtplib.SMTP(SMTP_HOST, SMTP_PORT) as server:
            server.set_debuglevel(1) # See full SMTP log in console
            server.starttls()
...
def fix_urls(item):
    ...
    if "images" in item and isinstance(item.get("images"), list):
        item["images"] = [fix_urls({"u": i})["u"] for i in item["images"]]
    return item
...
async def get_current_user(request: Request) -> dict:
    ...
    try:
        ...
        if not user_doc.exists: raise HTTPException(401, "User not found")
...
class LeadTransferIn(BaseModel):
    email: Optional[str] = None   # legacy: lookup by email
    target_id: Optional[str] = None  # preferred: lookup by user ID directly
...
class ServiceIn(BaseModel): name: str; category: str; description: Optional[str] = ""; price: float; duration_min: int; image_url: Optional[str] = None; membership_price: Optional[float] = None; reward_points: Optional[int] = 0; service_for: Optional[str] = "Men & Women"; hide_on_website: Optional[bool] = False
class MembershipIn(BaseModel): name: str; price: float; duration_days: int; reward_points_on_purchase: Optional[int] = 0; discount_on_services: Optional[float] = 0; discount_on_services_type: Optional[str] = "%"; discount_on_products: Optional[float] = 0; discount_on_products_type: Optional[str] = "%"; discount_on_packages: Optional[float] = 0; discount_on_packages_type: Optional[str] = "%"; reward_points_boost: Optional[str] = "1X"; min_reward_points_earned: Optional[int] = 0; condition: Optional[str] = "AND"; min_billed_amount: Optional[float] = 0
...
def register(data: RegisterIn, response: Response):
    ...
    uid = new_id()
    user_doc = {"id": uid, "name": data.name, "email": email, "password_hash": hash_password(data.password), "phone": data.phone or "", "role": "user", "points": 20, "created_at": now_iso()}
...
def login(data: LoginIn, response: Response):
    ...
    try:
        ...
        if not docs: raise HTTPException(401, "Invalid credentials")
        user = docs[0].to_dict()
        if not verify_password(data.password, user["password_hash"]): raise HTTPException(401, "Invalid credentials")
        ...
        if "Quota exceeded" in err_msg or "429" in err_msg:
            raise HTTPException(429, "Database quota exceeded. Please try again tomorrow or contact support.")
...
def approve_leave_request(rid: str, admin: dict = Depends(require_admin)):
    ...
    if not is_super:
        ...
        if current_status == "pending":
            doc_ref.update({"status": "branch_approved"})
            return {"ok": True, "status": "branch_approved"}
        else:
            raise HTTPException(400, "Branch Admin can only approve pending requests")
    else:
        # Superadmin
        if current_status == "branch_approved":
            ...
            raise HTTPException(400, f"Superadmin can only approve requests that are already Branch Admin approved (current: {current_status})")
...
async def check_attendance(user: dict = Depends(require_employee)):
    ...
    if docs:
        ...
        return {
            ...
            "recess_start_time": attendance_data.get("recess_start_time"),
            "recess_end_time": attendance_data.get("recess_end_time"),
            "half_day_penalty": attendance_data.get("half_day_penalty")
        }
    return {
        "verified": False,
...
async def start_recess(user: dict = Depends(require_employee)):
    ist_now = datetime.now(timezone.utc) + timedelta(hours=5, minutes=30)
    today = ist_now.strftime("%Y-%m-%d")
    time_str = ist_now.strftime("%H:%M:%S")
    ...
    attendance_data = docs[0].to_dict()
    if "recess_start_time" in attendance_data:
        return {"ok": True, "message": "Recess already started"}
        
    doc_ref.update({"recess_start_time": time_str})
    return {"ok": True}

@api.post("/attendance/recess/end")
async def end_recess(user: dict = Depends(require_employee)):
    ist_now = datetime.now(timezone.utc) + timedelta(hours=5, minutes=30)
    today = ist_now.strftime("%Y-%m-%d")
    time_str = ist_now.strftime("%H:%M:%S")
    ...
    docs = db.collection("attendance").where("user_id", "==", user["id"]).where("date", "==", today).limit(1).get()
    if not docs:
        raise HTTPException(400, "No attendance record found for today.")
        
    doc_ref = docs[0].reference
    attendance_data = docs[0].to_dict()
    if "recess_end_time" in attendance_data:
        return {"ok": True, "message": "Recess already ended"}
        
    start_time_str = attendance_data.get("recess_start_time")
    if not start_time_str:
        raise HTTPException(400, "Recess was never started.")
        
    start_dt = datetime.strptime(start_time_str, "%H:%M:%S")
    end_dt = datetime.strptime(time_str, "%H:%M:%S")
    duration_mins = (end_dt - start_dt).total_seconds() / 60.0
    
    half_day_penalty = duration_mins > 35
    
    doc_ref.update({
        "recess_end_time": time_str,
        "half_day_penalty": half_day_penalty
    })
    return {"ok": True, "half_day_penalty": half_day_penalty}
...
async def verify_checkout(data: AttendanceVerifyIn, user: dict = Depends(require_employee)):
    ...
    # Do not allow non-admin employees to check out before their working hours end
    if user.get("role") != "admin":
        wh_to = user.get("working_hours_to", "19:00")
        wh_to_minutes = None
        try:
            if " " in wh_to:  # e.g., "07:00 PM"
                t_part, ampm = wh_to.split()
                h, m = map(int, t_part.split(":"))
                if ampm.upper() == "PM" and h < 12:
                    h += 12
                elif ampm.upper() == "AM" and h == 12:
                    h = 0
                wh_to_minutes = h * 60 + m
            else:
                parts = list(map(int, wh_to.split(":")))
                if len(parts) >= 2:
                    wh_to_minutes = parts[0] * 60 + parts[1]
        except Exception:
            wh_to_minutes = 1140  # Fallback to 19:00 (7 PM)
            
        current_minutes = ist_now.hour * 60 + ist_now.minute
        if wh_to_minutes is not None and current_minutes < wh_to_minutes:
            display_h = wh_to_minutes // 60
            display_m = wh_to_minutes % 60
            display_ampm = "AM"
            if display_h >= 12:
                display_ampm = "PM"
                if display_h > 12:
            ...
            if display_h == 0:
                display_h = 12
            display_time = f"{display_h:02d}:{display_m:02d} {display_ampm}"
            raise HTTPException(400, f"You are not allowed to check out before your shift ends at {display_time}.")
...
def get_product(pid: str):
    ...
    if not doc.exists: raise HTTPException(404, "Product not found")
    return fix_urls(doc.to_dict())

@api.get("/products/{pid}/reviews")
def list_reviews(pid: str):
    docs = db.collection("reviews").where("product_id", "==", pid).stream()
...
def create_review(pid: str, data: ReviewIn, user: dict = Depends(get_current_user)):
    rid = new_id()
    review = {"id": rid, "product_id": pid, "user_id": user["id"], "user_name": user["name"], "rating": data.rating, "comment": data.comment, "created_at": now_iso()}
...
def list_stylists():
    ...
    if cached_stylists is None:
        docs = db.collection("users").where("role", "==", "service").stream()
        cached_stylists = []
        for d in docs:
            u = d.to_dict()
            name = u.get("name", "").strip()
            cached_stylists.append({
                "id": d.id,
                "name": name,
                "role": u.get("section") or "Service Team",
                "branch": u.get("branch")
            })
        cache_set("raw_stylists", cached_stylists, ttl=60)
...
def create_booking(data: BookingIn, user: dict = Depends(get_current_user)):
    bid = new_id()
    booking = {"id": bid, "user_id": user["id"], **data.model_dump(), "status": "pending", "created_at": now_iso()}
    db.collection("bookings").document(bid).set(booking)
    return booking

@api.get("/bookings/me")
def my_bookings(user: dict = Depends(get_current_user)):
    docs = db.collection("bookings").where("user_id", "==", user["id"]).stream()
    items = [d.to_dict() for d in docs]
    items.sort(key=lambda x: x.get("created_at", ""), reverse=True)
    return items
...
def create_order(data: OrderIn, user: dict = Depends(get_current_user)):
    ...
    points_used = min(data.points_used or 0, user_points)
    discount = points_used / 10
    
    final_total = max(0, total_price - discount)
    points_earned = int(final_total / 100 * 2)
    ...
    if data.add_to_wallet and not lead_ref:
        raise HTTPException(400, "Walk-in clients cannot add change to a wallet. Please select/create a client profile.")
    ...
    for update in items_to_update:
        ...
        if update["new_stock"] <= 5:
            low_stock_items.append(f"{update['name']} (Remaining: {update['new_stock']})")
            
    # Notify Admin if any item is low on stock
    if low_stock_items:
        items_list = "</li><li>".join(low_stock_items)
        email_body = f"""
        <div style='font-family: sans-serif; padding: 20px; border: 1px solid #eee;'>
            <h2 style='color: #d4af37;'>Low Stock Alert</h2>
            <p>The following items are running low on stock:</p>
            <ul><li>{items_list}</li></ul>
            <p>Please restock soon to avoid service disruption.</p>
        </div>
        """
        try:
    ...
    order = {
        ...
        "user_id": user["id"], 
        "user_name": user["name"],
        **data.model_dump(exclude={"items", "branch"}),
        "branch": order_branch,
        "items": items_detail,
        "total": final_total,
    ...
    db.collection("users").document(user["id"]).update({"points": new_points})
    
    db.collection("orders").document(oid).set(order)

    # Keep revenue_cache in sync so we never need a full rescan
    try:
    ...
    cache_bust("admin_stats")  # Bust cache so next dashboard load shows new order
...
async def facebook_webhook(request: Request):
    ...
    # Meta Webhooks send lead notifications in 'entry'
    if "entry" in data:
...
def webhook_meta_sync(key: Optional[str] = None):
    ...
    try:
        ...
        if "error" in me_resp:
            return {"status": "error", "stage": "me", "error": me_resp["error"], "token_debug": token_debug}
        page_id = me_resp.get("id")
        page_name = me_resp.get("name")
    except Exception as e:
        return {"status": "error", "stage": "me", "message": str(e), "token_debug": token_debug}
...
@api.get("/leads")
def list_leads(all: Optional[bool] = False, branch: Optional[str] = None, user: dict = Depends(require_employee)):
    ...
    if user["role"] == "sales":
        q = coll.where("assigned_to", "==", user["id"])
        if branch:
            q = q.where("branch", "==", branch)
        assigned_query = q.limit(1000).stream()
        ...
        raw_leads = [enrich_lead(d.to_dict()) for d in coll.order_by("created_at", direction="DESCENDING").limit(500).stream() if all or is_lead_doc(d.to_dict())]
        if branch:
            raw_leads = [l for l in raw_leads if l.get("branch") == branch]
        return raw_leads
    else:
...
def update_lead(lid: str, data: LeadUpdate, user: dict = Depends(require_employee)):
    ...
    if data.status is not None:
        ...
        current_lead = doc_ref.get().to_dict()
        if data.status == "dead" and current_lead.get("status") != "dead":
            dead_by_uids = current_lead.get("dead_by_uids", [])
            if user.get("role") == "sales" and user["id"] not in dead_by_uids:
                dead_by_uids.append(user["id"])
            update_data["dead_by_uids"] = dead_by_uids
            update_data["dead_at"] = now_iso()
        elif data.status != "dead" and current_lead.get("status") == "dead":
            update_data["dead_at"] = None
...
def update_lead_visit(lid: str, data: LeadVisitUpdate, user: dict = Depends(require_employee)):
    doc_ref = db.collection("leads").document(lid)
    lead = doc_ref.get()
    if not lead.exists:
        raise HTTPException(404, "Lead not found")
    ...
    if data.visit_date:
        ...
        note = {"text": f"Visit scheduled for {data.visit_date} at {data.visit_time}", "author": user["name"], "timestamp": now_iso()}
        update_data["notes"] = firestore.ArrayUnion([note])
        
    # Marking visit done
    elif data.liked is not None:
        if data.liked:
            ...
            note = {"text": msg, "author": user["name"], "timestamp": now_iso()}
        else:
    ...
    if update_data.get("status") == "dead" and lead_data.get("status") != "dead":
        dead_by_uids = lead_data.get("dead_by_uids", [])
        if user.get("role") == "sales" and user["id"] not in dead_by_uids:
            dead_by_uids.append(user["id"])
        update_data["dead_by_uids"] = dead_by_uids
        update_data["dead_at"] = now_iso()
    elif update_data.get("status") and update_data.get("status") != "dead" and lead_data.get("status") == "dead":
        update_data["dead_at"] = None
...
def transfer_lead(lid: str, data: LeadTransferIn, user: dict = Depends(require_employee)):
    doc_ref = db.collection("leads").document(lid)
    if not doc_ref.get().exists:
        raise HTTPException(404, "Lead not found")

    # Prefer target_id (direct ID lookup), fall back to email search
    target_user = None
    if data.target_id:
    ...
    # Update branch/section to match new assignee if they have them set
...
def log_call(lid: str, data: CallLogIn, user: dict = Depends(require_employee)):
    ...
    if lead_snap.exists:
        if update_data.get("status") == "dead" and lead_dict.get("status") != "dead":
            dead_by_uids = lead_dict.get("dead_by_uids", [])
            if user.get("role") == "sales" and user["id"] not in dead_by_uids:
                dead_by_uids.append(user["id"])
            update_data["dead_by_uids"] = dead_by_uids
            update_data["dead_at"] = now_iso()
        elif update_data.get("status") and update_data.get("status") != "dead" and lead_dict.get("status") == "dead":
            update_data["dead_at"] = None
...
def admin_stats(branch: Optional[str] = None, user: dict = Depends(require_admin)):
    ...
    u = users_query.count().get()[0][0].value

    bookings_query = db.collection("bookings")
    if branch:
        bookings_query = bookings_query.where("branch", "==", branch)
    b = bookings_query.count().get()[0][0].value

    orders_query = db.collection("orders")
    if branch:
        orders_query = orders_query.where("branch", "==", branch)
    o_count = orders_query.count().get()[0][0].value

    products_query = db.collection("products")
    if branch:
        products_query = products_query.where("branch", "==", branch)
    p_count = products_query.count().get()[0][0].value

    s_count = db.collection("services").count().get()[0][0].value

    # Revenue: read from cached settings doc to avoid full collection scan
    if not branch:
        ...
        if rev_doc.exists:
            ...
            revenue = sum(d.to_dict().get("total", 0) for d in o_totals)
            db.collection("settings").document("revenue_cache").set(
    ...
    # IST calendar date for labelling
    today = now_utc.astimezone(ist).strftime("%Y-%m-%d")

    # UTC boundaries for the IST calendar day
    # IST midnight = UTC midnight - 5h30m = previous UTC day 18:30:00
    ist_midnight = datetime.now(ist).replace(hour=0, minute=0, second=0, microsecond=0)
    utc_day_start = ist_midnight.astimezone(timezone.utc)
    utc_day_end   = utc_day_start + timedelta(hours=24) - timedelta(seconds=1)

    # Format matching now_iso() output: "YYYY-MM-DDTHH:MM:SS+00:00"
    today_start = utc_day_start.strftime("%Y-%m-%dT%H:%M:%S+00:00")
    today_end   = utc_day_end.strftime("%Y-%m-%dT%H:%M:%S+00:00")

    # 1. CRM Lead Sales for Today — only fetch leads updated today
    lead_todays_sales = 0.0
    daily_sales_details = []

    leads_query = db.collection("leads").where("updated_at", ">=", today_start).where("updated_at", "<=", today_end)
    if branch:
    ...
    # 2. Manual Sales for Today — filter by date field directly
    ...
    # 3. Orders for Today — UTC range query matching now_iso() format
    daily_services = 0.0
    ...
    # Fetch ALL today's orders (no branch filter in Firestore) so legacy untagged orders are included
    orders_query_today = db.collection("orders").where("created_at", ">=", today_start).where("created_at", "<=", today_end)
    orders_stream = orders_query_today.stream()
    for doc in orders_stream:
        o = doc.to_dict()
        # For branch admins: include if branch matches OR branch is missing (legacy)
        if branch and o.get("branch") and o.get("branch") != branch:
            continue
        notes = o.get("notes", "")
...
def admin_orders(limit: int = 200, branch: Optional[str] = None, user: dict = Depends(require_admin)):
    ...
    # Fetch enough records; we'll filter in Python so we catch legacy untagged orders too
    fetch_limit = limit if not active_branch else 1000
    docs = q.order_by("created_at", direction="DESCENDING").limit(fetch_limit).stream()
    all_orders = [d.to_dict() for d in docs]

    if active_branch:
        # Include orders matching this branch OR legacy orders with no branch tag
        return [o for o in all_orders if not o.get("branch") or o.get("branch") == active_branch][:limit]
    return all_orders
...
def admin_consultations(_: dict = Depends(require_admin)):
...
def admin_update_consultation(cid: str, data: dict, user: dict = Depends(require_admin)):
    ...
    # Do not allow modifying immutable system fields
    payload = {k: v for k, v in data.items() if k not in ["id", "created_by", "created_by_id", "created_at"]}
    doc_ref.update(payload)
    
    # Sync with Lead
    phone = payload.get("phone", old_data.get("phone", ""))
    if phone:
        if not phone.startswith("+"):
            phone = "+" + phone
        existing_leads = list(db.collection("leads").where("phone", "==", phone).limit(1).stream())
        if existing_leads:
            lead_ref = existing_leads[0].reference
            lead_update = {
                "name": payload.get("name", existing_leads[0].to_dict().get("name")),
                "updated_at": now_iso()
            }
            if "status" in payload:
                # Map consultation status to lead grade
                lead_update["grade"] = payload["status"]
            if "follow_up_date" in payload:
                lead_update["follow_up_date"] = payload["follow_up_date"]
            if "location" in payload:
                lead_update["branch"] = payload["location"]
            if "source" in payload:
                lead_update["source"] = payload["source"]
            
            # System audit note
            note_text = f"Consultation Form Updated by Admin.\nStatus: {payload.get('status')}\nBudget: {payload.get('budget_range')}\nNotes: {payload.get('notes')}"
            note = {"text": note_text, "author": user["name"], "timestamp": now_iso()}
            lead_update["notes"] = firestore.ArrayUnion([note])
            
            lead_ref.update(lead_update)
            
    return {"status": "success", "id": cid}


@api.get("/consultations")
def list_consultations(user: dict = Depends(require_employee)):
    coll = db.collection("consultations")
    if user["role"] == "admin":
        docs = coll.stream()
    else:
        docs = coll.where("created_by_id", "==", user["id"]).stream()
...
def get_admin_permissions(_: dict = Depends(require_admin)):
    cached = cache_get("admin_permissions")
    if cached is not None:
        return cached
    doc = db.collection("settings").document("admin_permissions").get()
    result = doc.to_dict() if doc.exists else {"allowed_tabs": "__ALL__"}
    cache_set("admin_permissions", result, ttl=3600)  # Cache for 1 hour
    return result
...
def set_admin_permissions(data: dict, user: dict = Depends(require_admin)):
    ...
    if not is_super:
        ...
        if perms != "__ALL__" and "add-permissions" not in perms:
            raise HTTPException(403, "Only Super Admin or authorized Admins can modify access permissions")
...
def admin_employees(_: dict = Depends(require_employee)):
...
def admin_payroll(month: Optional[str] = None, branch: Optional[str] = None, user: dict = Depends(require_admin)):
    ...
    start_str = filter_val
    if len(filter_val) == 7:
        end_str = filter_val + "-31T23:59:59"
    else:
        end_str = filter_val + "T23:59:59"
    ...
    emp_query = db.collection("users").where("role", "in", ["employee", "sales", "service", "receptionist"])
    if branch:
        emp_query = emp_query.where("branch", "==", branch)
    employees = [d.to_dict() for d in emp_query.stream() if d.to_dict().get("is_active") is not False]
    
    BASE_SALARY = 15000
    COMMISSION_RATE = 0.05
    
    # Fetch attendance records for the month
    att_query = db.collection("attendance").where("date", ">=", start_str[:10]).where("date", "<=", end_str[:10])
    attendance_records = [d.to_dict() for d in att_query.stream()]
    
    # Fetch orders for the month to calculate service provider commissions
    orders_query = db.collection("orders").where("created_at", ">=", start_str).where("created_at", "<=", end_str)
    all_orders = [d.to_dict() for d in orders_query.stream()]
    
    try:
        req_year = int(filter_val[:4])
        req_month = int(filter_val[5:7])
    except:
        now_d = datetime.now()
        req_year, req_month = now_d.year, now_d.month
        
    total_days = calendar.monthrange(req_year, req_month)[1]
    
    now_d = datetime.now()
    if now_d.year == req_year and now_d.month == req_month:
        days_to_check = min(total_days, now_d.day)
    else:
    ...
    for emp in employees:
        ...
        # Allow specific base salary override if present in DB
        base = emp.get("base_salary", BASE_SALARY)
        comm_rate = emp.get("commission_rate", COMMISSION_RATE)
        
        # Calculate individual commissions from orders
        service_sales = 0.0
        product_sales = 0.0
        package_sales = 0.0
            ...
            for it in o.get("items", []):
                if it.get("service_provider") == emp_id or it.get("service_provider_2") == emp_id:
                    ...
                    if it_type == "product":
                        ...
                        package_sales += line_total
                        package_count += qty
                    elif it_type == "membership":
                        membership_sales += line_total
                        membership_count += qty
                    else:
                        service_sales += line_total
                        service_count += qty
                        
        service_commission_inr = emp.get("service_commission_inr", 0)
        product_commission_rate = emp.get("product_commission_rate", 0.02)
        product_commission_inr = emp.get("product_commission_inr", 0)
        package_commission_rate = emp.get("package_commission_rate", 0.0)
        ...
        if emp.get("role") == "sales":
            ...
            if target > 0 and service_sales < target:
                ...
                if service_commission_inr > 0:
                    service_commission = service_commission_inr * service_count
                else:
                    service_commission = service_sales * comm_rate
                    
            if product_commission_inr > 0:
                product_commission = product_commission_inr * product_count
            else:
                product_commission = product_sales * product_commission_rate
                
            if package_commission_inr > 0:
                package_commission = package_commission_inr * package_count
            else:
                package_commission = package_sales * package_commission_rate
                
            if member_commission_inr > 0:
                membership_commission = member_commission_inr * membership_count
            else:
                membership_commission = membership_sales * member_commission_rate
                
        commission = service_commission + product_commission + package_commission + membership_commission
        
        # Calculate attendance
        emp_att = [a for a in attendance_records if a.get("user_id") == emp_id and a.get("status") == "present"]
        present_dates = set(a.get("date") for a in emp_att)
        
        allowed_weekoffs = emp.get("allowed_weekoffs", 2)
        
        absences = 0
        late_days = 0
        
        wh_from = emp.get("working_hours_from", "10:00")
        try:
            h, m = map(int, wh_from.split(":"))
            dt_cutoff = datetime(2000, 1, 1, h, m) + timedelta(minutes=10)
            cutoff_time = dt_cutoff.strftime("%H:%M:00")
        except:
            cutoff_time = "10:10:00"
        
        for day in range(1, days_to_check + 1):
            day_str = f"{req_year}-{req_month:02d}-{day:02d}"
            
            # Check for late arrival
            att_for_day = next((a for a in emp_att if a.get("date") == day_str), None)
            if att_for_day:
                time_str = att_for_day.get("time", "")
                if time_str and time_str > cutoff_time:
                    late_days += 1
                
                if att_for_day.get("half_day_penalty"):
                    absences += 0.5
            else:
                
                # Check if weekend for absence calculation
                day_date = datetime(req_year, req_month, day)
                if day_date.weekday() >= 5: # 5=Sat, 6=Sun
                    absences += 2
                else:
                    absences += 1
                        
        if absences <= allowed_weekoffs:
            unpaid_leaves = 0
            weekoffs_taken = absences
        else:
            unpaid_leaves = absences - allowed_weekoffs
            weekoffs_taken = allowed_weekoffs
            
        if unpaid_leaves >= 5:
            # Forfeit weekoffs, treat as unpaid leaves
            total_unpaid_leaves = absences
            weekoffs_cancelled = True
        else:
            total_unpaid_leaves = unpaid_leaves
            weekoffs_cancelled = False
            
        daily_wage = base / 30
        attendance_deduction = total_unpaid_leaves * daily_wage
        ...
        payroll_data.append({
            ...
            "service_commission_rate": comm_rate,
            ...
            "package_commission_rate": package_commission_rate,
...
def admin_reports(month: Optional[str] = None, branch: Optional[str] = None, user: dict = Depends(require_admin)):
    ...
    # 3. Stream leads with selected fields once and group/process in memory
    leads_query = db.collection("leads")
    if branch:
        leads_query = leads_query.where("branch", "==", branch)
    ...
    for emp in employees:
        ...
        report_data.append({
            ...
            "monthly_target": emp.get("monthly_target", 100000)
        })
    return report_data

@api.get("/admin/employees/{uid}/dashboard")
def get_employee_dashboard(uid: str, _: dict = Depends(require_admin)):
    emp_doc = db.collection("users").document(uid).get()
    if not emp_doc.exists: raise HTTPException(404, "Employee not found")
    emp = emp_doc.to_dict()
    
    today = now_iso()[:10]
    month = now_iso()[:7]
    
    leads = [d.to_dict() for d in db.collection("leads").where("assigned_to", "==", uid).select(["status", "created_at", "updated_at", "total_sale_amount", "follow_up_date", "payments"]).stream()]
    
    calls_docs = db.collection("calls").where("user_id", "==", uid).select(["timestamp"]).stream()
    today_calls = sum(1 for doc in calls_docs if doc.to_dict().get("timestamp", "").startswith(today))
    
    manual = [d.to_dict() for d in db.collection("manual_sales").where("employee_id", "==", uid).stream()]
    
    # Calculate CRM stats for sales role
    total_assigned = len(leads)
    converted = len([l for l in leads if l.get("status") == "converted"])
    token_received = len([l for l in leads if l.get("status") == "token received"])
    visited = len([l for l in leads if l.get("status") == "visited"])
    todays_leads = len([l for l in leads if l.get("created_at", "").startswith(today)])
    
    lead_monthly_sales = 0.0
    lead_todays_sales = 0.0
    for l in leads:
        for p in l.get("payments", []):
            if isinstance(p, dict):
                ...
                if ts.startswith(month):
                    lead_monthly_sales += p.get("amount", 0.0)
                if ts.startswith(today):
                    lead_todays_sales += p.get("amount", 0.0)

    monthly_sales = lead_monthly_sales + sum(m.get("amount", 0) for m in manual if m.get("date", "").startswith(month))
    todays_sales = lead_todays_sales + sum(m.get("amount", 0) for m in manual if m.get("date", "").startswith(today))
    
    # Overdues: leads with follow_up_date before today that are still open
    open_statuses = {"new", "in process", "visit", "visited", "token received"}
    overdues = len([l for l in leads if l.get("status", "").lower() in open_statuses and l.get("follow_up_date", "") and l.get("follow_up_date", "") < today])
    due_today = len([l for l in leads if l.get("status", "").lower() in open_statuses and l.get("follow_up_date", "") == today])
    opportunities = len([l for l in leads if l.get("status", "").lower() in open_statuses])
    ...
    for doc in orders_stream:
        ...
        is_current_month = created_at.startswith(month)
        
        for it in o.get("items", []):
            if it.get("service_provider") == uid or it.get("service_provider_2") == uid:
                qty = it.get("quantity", 0)
                price = it.get("price", 0.0)
                line_total = it.get("line_total", price * qty)
    ...
    return {
        ...
        "result": {
            ...
            "visited": visited,
            "monthly_sales": round(monthly_services_revenue, 2) if emp.get("role") != "sales" else monthly_sales,
            "monthly_target": monthly_target
        }
    }

@api.get("/admin/employees/{uid}/leads")
def get_employee_leads(uid: str, _: dict = Depends(require_admin)):
    docs = db.collection("leads").where("assigned_to", "==", uid).stream()
    return [d.to_dict() for d in docs]

@api.patch("/admin/employees/{uid}")
def update_employee(uid: str, data: dict, _: dict = Depends(require_admin)):
...
def log_manual_sale(data: ManualSaleIn, _: dict = Depends(require_admin)):
    ...
    db.collection("manual_sales").document(sid).set(sale)
    return sale
...
def admin_update_product_usage(uid: str, data: dict, user: dict = Depends(require_admin)):
    ...
    # Calculate stock adjustment if quantity changes
...
def admin_list_expenses(branch: Optional[str] = None, user: dict = Depends(require_admin)):
    ...
    results = [d.to_dict() for d in docs]
    if branch:
        # Only include expenses explicitly matching this branch
        results = [e for e in results if e.get("branch") == branch]
...
def list_service_categories(_: dict = Depends(require_admin)):
    docs = db.collection("service_categories").stream()
    categories = [d.to_dict().get("name") for d in docs]
    if not categories:
        services_docs = db.collection("services").stream()
        existing = list(set([s.to_dict().get("category") for s in services_docs if s.to_dict().get("category")]))
        if not existing:
            existing = ["Hair Cut", "Hair Color", "Facial", "Threading", "Waxing", "Spa", "Other"]
        for cat in existing:
            db.collection("service_categories").document(cat).set({"name": cat})
        categories = existing
    categories.sort()
    return categories
...
@api.post("/admin/service-categories")
def create_service_category(data: ServiceCategoryIn, _: dict = Depends(require_admin)):
    name = data.name.strip()
    if not name:
        raise HTTPException(status_code=400, detail="Category name cannot be empty")
    db.collection("service_categories").document(name).set({"name": name})
    return {"name": name}

@api.delete("/admin/service-categories/{name}")
def delete_service_category(name: str, _: dict = Depends(require_admin)):
    db.collection("service_categories").document(name).delete()
    return {"ok": True}
...
def admin_create_coupon(data: CouponIn, _: dict = Depends(require_admin)):
    cid = new_id()
    doc = {"id": cid, **data.model_dump(), "created_at": now_iso()}
    db.collection("coupons").document(cid).set(doc)
    return doc

@api.delete("/admin/coupons/{cid}")
def admin_delete_coupon(cid: str, _: dict = Depends(require_admin)):
    db.collection("coupons").document(cid).delete()
    return {"ok": True}
...
def admin_create_service(data: ServiceIn, _: dict = Depends(require_admin)):
    sid = new_id(); doc = {"id": sid, **data.model_dump(), "created_at": now_iso()}; db.collection("services").document(sid).set(doc)
...
def admin_create_membership(data: MembershipIn, _: dict = Depends(require_admin)):
    mid = new_id(); doc = {"id": mid, **data.model_dump(), "created_at": now_iso()}; db.collection("memberships").document(mid).set(doc); return doc
...
def admin_update_membership(mid: str, data: dict, _: dict = Depends(require_admin)):
    ...
    if not doc_ref.get().exists: raise HTTPException(404, "Membership not found")
...
def admin_delete_membership(mid: str, _: dict = Depends(require_admin)):
    db.collection("memberships").document(mid).delete(); return {"ok": True}
...
def admin_update_package(pid: str, data: dict, _: dict = Depends(require_admin)):
    ...
    if not doc_ref.get().exists: raise HTTPException(404, "Package not found")
...
def admin_delete_package(pid: str, _: dict = Depends(require_admin)):
    db.collection("packages").document(pid).delete(); return {"ok": True}

@api.post("/admin/signed-url")
def get_signed_url(data: dict, user: dict = Depends(require_admin)):
    try:
        filename = data.get("filename", "upload.bin")
        content_type = data.get("contentType", "application/octet-stream")
        ext = Path(filename).suffix or ".bin"
        path = f"uploads/{new_id()}{ext}"
        bucket = storage.bucket()
        blob = bucket.blob(path)
        url = blob.generate_signed_url(version="v4", expiration=timedelta(minutes=15), method="PUT", content_type=content_type)
        return {"uploadUrl": url, "path": path, "publicUrl": f"/api/files/{path}"}
    except Exception as e: raise HTTPException(500, f"Signed URL Error: {str(e)}")
...
async def admin_upload(file: UploadFile = File(...), user: dict = Depends(require_admin)):
    ...
    res = put_object(filename, data, content_type)
    return {"path": res["path"], "url": f"/api/files/{res['path']}"}


class AdminManualAttendanceIn(BaseModel):
    user_id: str
    date: str
    time: str
...
def admin_manual_attendance(data: AdminManualAttendanceIn, _: dict = Depends(require_admin)):
    user_doc = db.collection("users").document(data.user_id).get()
    if not user_doc.exists:
        raise HTTPException(404, "User not found")
    user = user_doc.to_dict()
    
    existing_docs = db.collection("attendance").where("user_id", "==", data.user_id).where("date", "==", data.date).limit(1).get()
    if existing_docs:
        ...
        if data.checkout_time:
            ...
            update_data["checkout_time"] = firestore.DELETE_FIELD
        doc_ref.update(update_data)
        # Fetch fresh doc
        updated_doc = doc_ref.get().to_dict()
        return {"ok": True, "attendance": updated_doc}
    else:
        aid = new_id()
        doc = {
            "id": aid,
        ...
        if data.checkout_time:
            doc["checkout_time"] = data.checkout_time
        db.collection("attendance").document(aid).set(doc)
        return {"ok": True, "attendance": doc}

class AdminKioskIn(BaseModel):
    user_id: str
    is_checkout: bool
    photo_base64: str
    latitude: float
...
@api.post("/admin/attendance/kiosk")
async def admin_kiosk_attendance(data: AdminKioskIn, _: dict = Depends(require_admin)):
    user_doc = db.collection("users").document(data.user_id).get()
    if not user_doc.exists:
        raise HTTPException(404, "Employee not found")
    user = user_doc.to_dict()
    ...
    if data.is_checkout:
        ...
        doc_ref.update(update_doc)
        return {"ok": True, "message": "Checked out successfully", "attendance": {**docs[0].to_dict(), **update_doc}}
    else:
        if docs:
            return {"ok": True, "message": "Already checked in today", "attendance": docs[0].to_dict()}
        aid = new_id()
        doc = {
            "id": aid,
            "user_id": data.user_id,
...
def serve_file(filename: str):
    ...
    if IS_VERCEL:
        ...
        for cand in [raw, raw.replace("/", "_"), raw.split("/")[-1]]:
            p = UPLOADS_DIR / cand
...
class AdminServiceRowIn(BaseModel):
    service_id: str
    service_name: Optional[str] = ""
    category: Optional[str] = ""
    stylist_id: Optional[str] = None
    stylist_name: Optional[str] = "Any available"
...
def admin_list_appointments(admin: dict = Depends(require_admin)):
    branch = admin.get("branch")
    ...
    for d in docs:
        ...
        if branch and bk.get("branch") and bk.get("branch") != branch:
            continue
        # Also show bookings without a branch field to avoid hiding old data
        items.append(bk)
    items.sort(key=lambda x: (x.get("date",""), x.get("created_at","")), reverse=True)
    return items

@api.post("/admin/appointments")
def admin_create_appointment(data: AdminAppointmentIn, admin: dict = Depends(require_admin)):
    bid = new_id()
    rows = []
    for row in data.service_rows:
        rows.append({
            ...
            "stylist_name": row.stylist_name or "Any available",
    ...
    booking = {
        ...
        "date": data.date,
        "services": rows,
        # Backward-compat single-service fields (first row)
        "service_id": rows[0]["service_id"] if rows else "",
        "service_name": ", ".join(r["service_name"] for r in rows),
